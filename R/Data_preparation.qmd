---
title: "Indoor dust bacterial and fungal microbiota composition and allergic diseases: a scoping review"
...
`countries` now has `r length(countries)` columns.
...
`collectors` has `r count(data)` rows. 
...
`environmental_determinants` has `r count(data)` rows. 
...
write.csv(environmental_determinants, file.path(csv_folder, "environmental_determinants.csv"), row.names = FALSE)
...
```
  ...
  - Becker, Minka, Deckmyn. A (2023). _maps: Draw Geographical Maps_. R package version 3.4.2, <https://CRAN.R-project.org/package=maps>.
